...
const MAX_FINDING_ID_CHARS = 64;
const MAX_CLAIM_CHARS = 8_000;
const MAX_VERIFICATION_CHARS = 4_000;
...
function validateRequest(request) {
  ...
  const requestBytes = Buffer.byteLength(JSON.stringify(request), 'utf8') + 1;
  if (requestBytes > MAX_REQUEST_BYTES) {
    throw new Error(`review request exceeds the ${MAX_REQUEST_BYTES}-byte transport limit`);
  }

  return { ...evidenceMeta, requestBytes };
}
...
function validateConfig(config) {
  ...
  for (const [index, value] of args.entries()) {
    const secretReason = scanSecretText(value);
    if (secretReason) {
      throw new Error(
        `config.args[${index}] looks like a high-confidence secret (${secretReason}); pass credentials through --pass-env instead`
      );
    }
  }
